...
class RxExecTask: RxExecFullyAttachableType, RxExecLaunchedClassType {
    ...
    let linesErr: Observable<String>
    
    fileprivate let _errObservable: PipeOutObservable
    let stdErr: Observable<Data>
    ...
    internal init(descriptor: RxExecDescriptor) {
        ...
        self._errObservable = PipeOutObservable(pipe: errPipe)
        self.stdErr = self._errObservable.asObservable()
        self.linesErr = TextStreamProducer(self.stdErr).asObservable()
    ...
    internal func launch() {
        self.task.terminationHandler = { [weak self] (task) -> Void in
            ...
            this._outObservable.complete(true)
            this._errObservable.complete(true)
